...
class Student():
    ...
    def save(self):
        ...
        if self.id:
            ...
            self.ds.execute(query,(self.name,self.email,self.dob,self.password,self.id))
        else:
            #save into database
            query = f"INSERT INTO {self.__class__.TABLE_NAME} (name,email,dob,password) VALUES(%s,%s,%s,%s)"
            self.ds.execute(query,(self.name,self.email,self.dob,self.password))
            results = self.ds.execute(f"SELECT MAX(marticule) FROM {self.__class__.TABLE_NAME}")
