...
class Student():
    ...
    def save(self):
        ...
        if self.id:
            query = f"UPDATE {self.__class__.TABLE_NAME} SET name=%s ,email=%s ,password=%s WHERE teacher id=%s"
            self.ds.execute(query,(self.name,self.email,self.password,self.id))
        else:
            #save into database
            ...
            results = self.ds.execute(f"SELECT MAX(teacher id) FROM {self.__class__.TABLE_NAME}")
    ...
    def read(self,id=None):
        if id:
            query = f"SELECT * FROM {self.__class__.TABLE_NAME} WHERE teacher id=%s"
    ...
    def delete(self):
        #delete by id
        if self.id:
            query = f"DELETE FROM {self.__class__.TABLE_NAME} WHERE teacher id=%s"
            self.ds.execute(query,(self.id,))
